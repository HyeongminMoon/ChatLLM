...
nohup python fastchat/serve/vllm_worker.py --model-path /workspaces/disk0/data/llm_weights/Gugugo-koen-7B-V1.1/ --controller-address http://localhost:21001 --port 31009 --worker-address http://localhost:31009 --num-gpus 1 --max-num-batched-tokens 6120 --host 0.0.0.0 --limit-worker-concurrency 1024 > /dev/null 2>&1 &
