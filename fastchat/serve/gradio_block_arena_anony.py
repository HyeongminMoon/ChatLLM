...
def clear_history(request: gr.Request):
    ...
    return (
        [None] * num_sides + [None] * num_sides + anony_names + [""] + [disable_btn] * 7
    )
...
def stop_response(state0, state1,
                  model_selector0, model_selector1,
                  request: gr.Request): 
    logger.info(f"stop (anony). ip: {request.client.host}")
...
def build_side_by_side_ui_anony(models):
    notice_markdown = """
    - 익명의 모델과 대화하고 더 나은 모델에 투표하세요.
    - 평가하기 전 여러 번 대화할 수 있습니다.
