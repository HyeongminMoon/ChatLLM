...
async def api_list_models(request: Request):
    ret = requests.post(worker_controller_url + "/list_models")
    return ret.json()


@app.get("/refresh_all_models")
async def api_refresh_models(request: Request):
    ret = requests.post(worker_controller_url + "/refresh_all_workers")
...
async def api_model_details(request: Request):
    ...
    if not_exists:
        return {
            "err_code": PluginErrorCode.INVALID_INPUT,
            "err_msg": f"invalid input: {str(not_exists)}",
        }
    ...
    ret = requests.post(model_url + "/model_details")
    return ret.json()
...
async def api_worker_get_conv_template(request: Request):
    ...
    model_url = ret.json()["address"]
    ret = requests.post(model_url + "/worker_get_conv_template")
    return ret.json()
...
async def api_count_token(request: Request):
    ...
    model_url = ret.json()["address"]
    ret = requests.post(model_url + "/count_token", json=params)
    return ret.json()
...
async def api_worker_generate(request: Request):
    ...
    model_url = ret.json()["address"]
    ret = requests.post(model_url + "/worker_generate", json=params)
    return ret.json()
...
async def api_worker_stop_stream(request: Request):
    ...
    model_url = ret.json()["address"]
    ret = requests.post(model_url + "/worker_stop_stream", json=params)
    return ret.json()
...
async def api_websearch_run_all(request: Request):
    params = await request.json()
    not_exists = valid_input_checker(
        params, ["model_name", "session_id", "search_keyword"]
    )
    if not_exists:
        return {
            "err_code": PluginErrorCode.INVALID_INPUT,
...
async def api_retrievalqa_get_result(request: Request):
    ...
    if vector_store == PluginErrorCode.VECTOR_STORE_NOT_EXIST:
        return {
            "err_code": PluginErrorCode.VECTOR_STORE_NOT_EXIST,
...
async def api_ts_translate(request: Request):
    ...
    result = translate(sentence, ts_tool=ts_tool, ts_lang=to_lang, from_lang=from_lang)
