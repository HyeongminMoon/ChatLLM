...
class ChromaCollector:
    def __init__(self, embedder_name: str = "ddobokki/klue-roberta-base-nli-sts-ko-en"):
        ...
        self.collection = self.chroma_client.create_collection(
            name="context", embedding_function=self.embedder.embed
        )
        self.ids = []
    ...
    def get_sorted(self, search_strings: list[str], n_results: int) -> list[str]:
        documents, ids, distances = self.get_documents_ids_distances(
    ...
    def apply_time_weight_to_distances(
        self, ids: list[int], distances: list[float], time_weight: float = 1.0
    ) -> list[float]:
        if len(self.ids) <= 1:
            return distances.copy()

        return [
            distance * (1 - _id / (len(self.ids) - 1) * time_weight)
...
def feed_urls_into_collector(
    collector, urls, num_workers=5, chunk_len=400, chunk_sep="", strict=False
):
    for url in urls:
...
def feed_webquery_into_collector(
    collector,
    query,
    num_workers=5,
    chunk_len=400,
...
def get_websearch_result(
    ...
    for _data in search_data:
        ...
        if extracted:
            temp_chucks = [
                extracted[i : i + chunk_len]
                for i in range(0, len(extracted), chunk_len)
            ]
